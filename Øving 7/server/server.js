...
//STUFF: https://egghead.io/lessons/node-js-build-a-simple-node-js-web-server-with-docker
...
app.delete('/customers', (request, response) => {
  for (var i = 0; i < customers.length; i++) {
    if (customers[i].id == request.body.id) {
  ...
  //If the program reaches this place the customer is not found and not deleted.
  response.sendStatus(404);
});

app.put('/customers/changeCustomerName', (request, response) => {
  for (var i = 0; i < customers.length; i++) {
    if (customers[i].id == request.body.id) {
...
app.put('/customers/changeCustomerCity', (request, response) => {
  for (var i = 0; i < customers.length; i++) {
    if (customers[i].id == request.body.id) {
